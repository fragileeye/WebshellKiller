...
/*
    edited by suiyan:
    1. copied from the project and thanks: php-malware-finder(
        https://github.com/nbs-system/php-malware-finder/tree/master/php-malware-finder)
        without the work of the author, I couldn't come up with the idea, thanks again.

    2. remove white list file.

    3. change/add some rule and make it stickly.
...
rule ObfuscatedPhp
{
    strings:
        ...
        $variable_variable2 = /(@?\$\s*\w+\s*\(){3,}[^;]+/ //added by suiyan for oneword shell

        $too_many_chr = /(chr\([\d]+\)\.){8}/  // concatenation of more than eight `chr()`
        $too_many_chr2 = /(chr[^.]+\.|substr[^.]+\.){4,}[^;]+/ nocase //added by suiyan for oneword shell

        $concat = /(\$[^\n\r]+\.){5}/  // concatenation of more than 5 words
        $concat_with_spaces = /(\$[^\n\r]+\. ){5}/  // concatenation of more than 5 words, with spaces
        $concat_with_dots = /(\s*(\"|\')\s*\w+\s*(\"|\')\s*\.){5}/ //added by suiyan
        $concat_with_dots2 = /(\([^.]+\.){6}/ //added by suiyan

        $var_as_func = /\$_(GET|POST|COOKIE|REQUEST|SERVER)\s*\[[^\]]+\]\s*\(/
        $var_as_func2 = /\$\s*(\w+|\{\s*\w+\s*\})\s*(\(|\{)\s*\$[^;]+;[^=?]*\?>/  //added by suiyan
...
rule special_types //added by suiyan
{
    strings:
        $ = "6776666E286763736A38346466656871646A2A2464524F58565B2C7C302C5F292E" nocase //cve oneword
        $ = "greeting: !{$str}" //yaml_parse 
        $ = "~\xbe\xac\xac\xba\xad\xab" nocase //strange encoding

    condition:
        any of them
}

rule mal_types //added by suiyan
{
    strings:
        $mal_user_func = /\b(call_user_func(_array)?)\s*\(\s*create_function/ nocase 
        $mal_dolar_ = /\$(\w+)\s*\(\$(\w){1,}\[\$(\w){1,}[^;]+;/ 
        $mal_array = /\$array\s*(\[[^\]]+\]){1,}\(\s*\$[^;]+;[^=?]*\?>/ nocase 
        $mal_class = /(eval|assert|passthru|exec|include|require|require_once|system|pcntl_exec|shell_exec|`|ob_start)\s*\(\$this-\>[^;]+;/ nocase //evil class
        ...
        $mal_symbs = /[^\w\s]{100}/  //string with + - {} () [] too long
        $mal_base64 = /(=[^"']*["'][0-9a-zA-Z+\/]+={1,2}['"][^=]+){5}/ //too many base64 encoding string
...
rule DodgyPhp
{
    strings:
        ...
        $execution = /\b(eval|assert|passthru|exec|include|require|require_once|system|pcntl_exec|shell_exec|base64_decode|`|ob_start|call_user_func(_array)?)\s*\(\s*([^.]+\.)?(base64_decode|php:\/\/input|str_rot13|gz(inflate|uncompress)|getenv|pack|\\?\$_(GET|REQUEST|POST|COOKIE|SERVER))/ nocase  // function that takes a callback as 1st parameter
       
        $execution2 = /\b(array_map|array_filter|array_reduce|array_walk(_recursive)?|array_walk|assert_options|uasort|uksort|usort|preg_replace_callback|iterator_apply|mb_ereg_replace|preg_filter|register_shutdown_function|register_tick_function)\s*\(\s*[^,]*,?\s*(substr_replace|strrev|base64_decode|php:\/\/input|str_rot13|gz(inflate|uncompress)|getenv|pack|\\?\$_(GET|REQUEST|POST|COOKIE|SERVER))/ nocase  // functions that takes a callback as 2nd parameter
...
rule DangerousPhp
{
    ...
    condition:
        /* 5 maybe too careless, any of them maybe more appropriate*/
...
private rule hex
{
    strings:
        ...
        $assert = "\\x61\\x73\\x73\\x65\\x72\\x74" nocase
...
rule Websites
{
    strings:
        ...
        $ = "mumaasp.com" nocase
        $ = "http://mc.kylem.in/404update.php" nocase
    condition:
        (any of them) 
}
